...
import { Grid , Card, Container, Typography, CardContent, CardActions, Button } from '@material-ui/core';
import './index.css';
            ...
            <Grid className ="items" item sm={3} key = {data.id} >
                <Card>
                    <CardContent id={ `${data.id}Card` } >
                        <Typography variant="h6" > {data.coffie} : {data.amount} </Typography>
                        ...
                        <div id={data.id} className = "Add-cart-btn">
                            <CardActions  >
                                <Button onClick = {() => addCart(data)} variant="outlined" color="primary">+</Button> 
                                <p className = "count"> {count} </p>
                                <Button onClick = {() => removeCart(data)} variant="outlined" color="primary">-</Button>
                            </CardActions>
                        </div>
                    </CardContent>
                </Card>
